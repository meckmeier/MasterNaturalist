# Upload process
You can download a sample CSV format to identify the columns that most easily will import into the system. There are rules you need to follow to fill out the columns or the data will not go in correctly. Please refer to the column heads and sample values below:
...
"activity_url"| If you activity is digital you must include this field. It will also be used at the activity level for the more information link.|
"contact_email"| For more information you should provide either the activity url or a contact email. One of these is required.|
...
"session_format"| REQUIRED: Type an "o" if the activity is online, or "b" if it is online AND also in person. Use "i" if the activity is in person only. or "s" if the location is self-directed.|
